/**
 * "Continue with Google" button for the login and register pages.
 *
 * This is a plain link, not a fetch call: OAuth needs a full browser navigation
 * so Google can show its own consent screen and redirect back.
 *
 * It renders only when the server actually has Google credentials, so an
 * unconfigured deployment never shows a button that dead-ends.
 */
...
function GoogleMark() {
  // Google's brand guidelines require their official four-colour mark.
  return (
...
export default function SocialAuthButtons({ mode }: { mode: "login" | "register" }) {
  ...
  if (!data?.providers?.includes("google")) return null;
